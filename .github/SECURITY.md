...
LucidScript is a **brand surface** over [AffineScript](https://github.com/hyperpolymath/affinescript): it carries examples, documentation, and a thin `lucid` shim CLI. It ships no compiler, type checker, or runtime of its own — those live in `affinescript`.
...
- For issues specific to **this repository** (the `bin/lucid` shim, examples, or docs), email **j.d.a.jewell@open.ac.uk** with details and a reproduction. Please do not open a public issue for a security report.
...
In scope: the `bin/lucid` shim, repository workflows, and example sources.
