...
sed -i 's/{{project}}/my_project/g' $(grep -rl '{{project}}' .)
sed -i 's/lucidscript/my-project/g' $(grep -rl 'lucidscript' .)
...
| `lucidscript` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
