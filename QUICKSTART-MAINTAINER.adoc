...
// Replace rsr-template-repo, lucidscript, {{DEPS}} with actuals
...
| `$PREFIX/share/lucidscript/`
| Data files, assets

| `$PREFIX/share/doc/lucidscript/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/lucidscript/config.toml`

Fallback: `$HOME/.config/lucidscript/config.toml`
...
Or via OPSM: `opsm update lucidscript`
...
[source,bash]
----
just install --prefix=/opt/lucidscript-instance1 --config=/etc/lucidscript/instance1.toml
just install --prefix=/opt/lucidscript-instance2 --config=/etc/lucidscript/instance2.toml
----
